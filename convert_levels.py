...
MAP_SRC_DIR = os.path.join(BASE_PATH, 'data/levels')
MAP_COMPILED_DIR = os.path.join(BASE_PATH, 'data/levels/compiled')
TILE_LAYER_NAMES = ["floor", "walls", "wall_tops"]
GRID_LAYER_NAMES = []
#--------------------------------------
...
def flatten_ogmo_tilemaps(ogmo_path, ogmo_flattened_path):
    ...
    for tile_layer_name in TILE_LAYER_NAMES:
        if len(ogmo_dom.getElementsByTagName(tile_layer_name)) < 1:
             continue
        ...
        for y in range(0, heightInTiles):
            for x in range(0, widthInTiles):
                tileId = tiles.get(str(x) + '@' + str(y), '-1')  #altered to print out -1 for non-existent tiles; usually just 0
                map_data[tile_layer_name]['tiles'] += tileId
                if x + 1 in range(0, widthInTiles):
					map_data[tile_layer_name]['tiles'] += ","
            map_data[tile_layer_name]['tiles'] += "\n"
    ...
    for grid_layer_name in GRID_LAYER_NAMES:
        gridString = ogmo_dom.getElementsByTagName(grid_layer_name)[0].firstChild.data;
        flattenedGridString = ""
        for i in range(0, len(gridString)):
            flattenedGridString += gridString[i]
            if i + 1 in range(0, len(gridString)) and gridString[i + 1] != "\n" and gridString[i] != "\n":
                flattenedGridString += ","			
		
        ogmo_dom.getElementsByTagName(grid_layer_name)[0].childNodes[:] = []
        flattenedGridNode = ogmo_dom.createTextNode(flattenedGridString)
        ogmo_dom.getElementsByTagName(grid_layer_name)[0].appendChild(flattenedGridNode)
